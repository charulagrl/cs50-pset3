...
int main(int argc, string argv[])
{
    ...
    while (true)
    {
        ...
        if (word != NULL)
        {
            ...
            // convert word to all caps to ignor case sensitivity
...
bool lookup(string word)
{
    // create variable for lowest word (alphabetically) being searched 
    int low = 0;
    
    // create variable for highest word (alphabetically) being searched 
    int high = WORDS - 1;
    
    // as long as low hasn't surpassed high, keep conducting the binary search    
    while (low <= high) 
    {
        // create variable for the the midpoint
        int mid = (low + high) / 2;
        
        // if the midpoint is the word and the word hasn't been found, return true, else return false      
        if (strcmp(word, dictionary.words[mid].letters) == 0)

        {   
            // if the word hasn't been found, mark thezz
            if (dictionary.words[mid].found != true)
            {
                // mark the word as found
                dictionary.words[mid].found = true;
                
                // return true so user can score
                return true;
            }
            
            // if the word has been found, return false
            else
                return false;                
        }
        
        // if word is > the midpoint, look in the upper half of the dictionary        
        else if (strcmp(word, dictionary.words[mid].letters) > 0)
            // + 1 because we know the word isn't in the middle
            low = mid + 1;
        
        // if the word < the midpoint, look in the lower half of the dictionary        
        else
            //- 1 because we know the word isn't in the middle
            high = mid - 1;
    }
    
    // return false if the word isn't found after the binary search   
    return false;
}
...
void scramble(void)
{
    ...
    for (int row = 0; row < DIMENSION; row++)
    {
        for (int col = 0; col < DIMENSION; col++)
        {
            // rotates grid's 90 degrees clockwise and stores in temp
