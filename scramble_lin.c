...
bool lookup(string word)
{
    ...
    {   
        // if the word matches and is found for the first time
        if ((strcmp(word, dictionary.words[i].letters) == 0) && (dictionary.words[i].found != true))
            {
            // mark the word as found
            dictionary.words[i].found = true;
            
            // return true to show that the word has been found for the first time
            return true;
            }

    }
    
    // if the word is not found for the first time or is not in the dictionary, return false
    return false;
}
